...
@section('main-content')
    ...
    <section class="section">
        ...
        <div class="section-body">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select name="variant_id" id="Variants" class="form-control select2 @error('variant_id') is-invalid @enderror">
                                        <option value="">{{ __('Select Variants') }}</option>
                                        @if(!blank($variants))
                    ...
                    <div class="card-body" id="printablediv">
                        @if(isset($productVariant))
                                ...
                                <div class="item barcodeStyle">
                                    ...
                                    <span class="barcode_price">{{__('Price')}} {{currencyFormat($product->price + $productVariant->price)}}</span>
                        ...
                        <div class="printBarcode">
                            @foreach(range(0,$set_quantity) as $number)
                                <div class="item barcodeStyle">
                                    ...
                                    <span class="barcode_price">{{__('Price')}} {{currencyFormat($productBarcode->price)}} @if(isset($productBarcode->tax))/ {{$productBarcode->tax->name}}@endif</span>
                                    <span class="barcode_image">
            ...
            @else
              <h4 class="text-danger">{{ __('The Barcode/Label data not found') }}</h4>
                        @endif
                    </div>
                </div>
            @endif
