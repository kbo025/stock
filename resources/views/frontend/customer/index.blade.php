...
@section('content')
    <div class="container-padding">
        <div class="row">
            <div class="col-lg-6">
                <div>
                    <h1 class="header-title">{{__('levels.find_your_balance')}}</h1>
                    <p class="header-description">{{__('levels.give_your_whatsapp_number')}}</p>
                    <form id="myForm" action="{{ route('check-balance') }}" method="POST" >
                        @csrf
                        <div class="form-group">
                            <input type="number" class="form-control @error('phone') is-invalid @enderror" id="phone" pattern="+[7-9]{2}-[0-9]{3}-[0-9]{4}"  name="phone" value="{{old('phone',$phone)}}" placeholder="Phone Number">
                            @error('phone')
                        ...
                        <button type="submit" class="btn btn-primary">{{__('levels.check_balance')}}</button>
                    </form>
                </div>
            </div>
                        ...
                        @elseif(!$details)
                            <div class="row g-4">
                                <div class="col d-flex align-items-start border border-warning bg-white rounded">
                                    <p align="center" class="not-data text-danger  m-auto py-3 h4">{{__('levels.sorryno_customer_available!')}}</p>
                                </div>
                            </div>
                    </article>
                    @else
