...
@section('content')
    <div class="container-padding">
        <div class="row">
            <div class="col-lg-6">
                <h1 class="header-title">{{__('levels.register_with_us')}}</h1>
                <p class="header-description">{{__('levels.register_by_your_whatsapp_number')}}</p>
                ...
                <form id="payment-form" action="{{ route('check-in.step-one.store') }}" method="POST" >
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <input type="number" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" pattern="+[7-9]{2}-[0-9]{3}-[0-9]{4}"  value="{{old('phone')}}" placeholder="Phone Number">
                            @error('phone')
                            <span class="small invalid-feedback text-warning">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            @if(auth()->user())
                            <input type="number" step="0.01" class="form-control @error('deposit_amount') is-invalid @enderror" id="deposit_amount" name="deposit_amount" value="{{old('deposit_amount')}}" placeholder="Deposit Amount">
                        ...
                        <div class="col-md">
                            <div class="reg-form form-floating border-black">
                                <div class="payment-card-css mb-3">
                                    <label id="card-element-label">{{ __('levels.credit_or_debit_card') }}</label>
                                    <div class="mt-1">
                    ...
                    <button type="submit" class="btn btn-primary">{{__('levels.take_membership')}}</button>
                </form>
            </div>
